...
from sklearn.cross_validation import StratifiedKFold, LeaveOneOut, ShuffleSplit
...
##### Models to evaluate #######################################################
model_eval     = ['rbfsvm','linsvm','rf','xgb']
...
linsvm_weightfn = lambda model: model.coef_.squeeze()

# RBF SVM
rbfsvm_tuned = {'C':svmC,'gamma':logspace(-7,4,12)}
rbfsvm_default = {
    'C':1.0,'kernel':'rbf','gamma':1,'verbose':train_verbose,
    'max_iter':10e5,'probability':False,'random_state':train_state
}
# rbf weightfn in dual space, inaccessible in primal, and thus == nan
rbfsvm_weightfn = lambda model: ones(model.support_vectors_.shape[1])*nan 
...
rf_weightfn = lambda model: model.feature_importances_


model_params  = {} # map from model_id -> tuple(default, tuning_params, weightfn)
...
if pred_mode == 'clf':
    ...
    model_params['xgb']  = (XGBClassifier(**xgb_default),[xgb_tuned],xgb_weightfn)
    ...
    if 'xgb' in model_eval:
        print 'Warning: model XGB not implemented for regression, skipping'
        model_eval.remove('xgb')
    ...
    scorefn['mse'] = lambda te,pr,_: mean_squared_error(te,pr)
    

model_params['linsvm'] = (LinSVM(**linsvm_default),[linsvm_tuned],linsvm_weightfn)
model_params['rbfsvm'] = (RBFSVM(**rbfsvm_default),[rbfsvm_tuned],rbfsvm_weightfn)
model_params['rf']     = (RF(**rf_default),[rf_tuned],rf_weightfn)

def tuned_fit(clf,tuned_params):
    '''
    return a dict of the parameters in tuned_params that were tuned to clf
    '''
    clf_fit = {}
...
def model_train(X_train,y_train,model_clf,model_tuned):
    '''
    train/tune a model from the model_params dict
    '''
    clf = clone(model_clf)
    
    with warnings.catch_warnings():
        warnings.simplefilter("ignore") # disable sklearn convergence warnings
        if model_tuned is not None or len(model_tuned) == 0: # tuned params if desired
...
def discrim_cv(X,y,cv,output={}):
    ...
    for model_id in model_eval:
        print 'Evaluating %s model "%s" on %d %d-dimensional samples (%s)'%(pred_mode,
                                                                              model_id,
                                                                              N,n,pstr)
        model_clf,model_tuned,model_weightf = model_params[model_id]
        ...
        output[model_id] = {'models':models,'model_tuned':model_tuned,
                            'preds':preds,'scores':scores}
    return output

def discrim_state(input_statefile,output_statefile,update_output=False):
    '''
    run discrim_cv for an experiment input statefile and serialize the output 
    to output_statefile, scales samples using scaling_method, and excludes
    ...
    X_exp, y_exp = input_state['X_exp'],input_state['y_exp']

    multi_output = len(y_exp.shape) > 1 and min(y_exp.shape) > 1        
    ...
    X_exp = scale_fn(X_exp)
        
    if multi_output:
        y_exp = y_exp[y_exp.sum(axis=1)>0,:]
    else:
        y_exp = y_exp.ravel()

    check_X_y(X_exp,y_exp,multi_output=multi_output)        
        
    # exclude unlabeled samples (assume y==0 is unlabeled)
    labmask = (y_exp != 0)
    y = y_exp[labmask].copy()    
    X = X_exp[labmask,:].copy()    
...
def discrim_output_weights(exp_name,exp_dir=output_dir):    
    input_statefile  = pathjoin(exp_dir,exp_name+'_input.pkl')    
    output_statefile = pathjoin(exp_dir,exp_name+'_output.pkl')    
    try:
    ...
    for model_id in output_state['model_eval']:
        ...
        weight_fn = model_params[model_id][-1]
        model_weights = []
        for i,model in enumerate(models):
            if model_id == 'xgb': # fix stupid xgb dimensionality issue
                w = weight_fn(model,input_state['X_exp'].shape[1])
            else:
                w = weight_fn(model)
            model_weights.append(w)

        model_weights = asarray(model_weights)
        model_mean    = mean(model_weights,axis=0)
        model_std     = std(model_weights,axis=0)
        
        output_weights[model_id] = {'weights':model_weights,
                                    'mean':model_mean,
                                    'std':model_std}
...
if __name__ == '__main__':
    ...
    if pred_mode == 'clf':       
        ...
        y_exp = c_[y_exp==1,y_exp==0].astype(float)
        
    discrim_exp(X_exp,y_exp,'digits5v8',exp_dir='discrim_test')
